...
import { modeBooks, targetMode, SAMPLE_COUNT, samplesAtLeast, SUPPORTED_TARGETS } from '../stacks-3d-home/engine-contract.mjs';

import { GAME_MODES, boostedPayoutUnits } from '../stacks-3d-home/game-modes.mjs';
...
const modeDefinitions = GAME_MODES.flatMap(mode => SUPPORTED_TARGETS.map(target => ({
  name: targetMode(target, mode.id), target, cost: 1, rows: modeBooks(target, mode.id), modeId: mode.id,
})));
for (const [targetIndex, definition] of modeDefinitions.entries()) {
  const { name, target, cost, rows, modeId } = definition;
  ...
  for (const { book, weight: count } of rows) {
    ...
    assert.equal(book.payoutMultiplier, book.events[0].resultUnits >= target ? boostedPayoutUnits(target, modeId) : 0);
    weight += count;
  ...
  assert.equal(wins, samplesAtLeast(boostedPayoutUnits(target, modeId)));
...
const report = {
  ...
  targetRange: { minimum: Math.min(...SUPPORTED_TARGETS) / 100, maximum: Math.max(...SUPPORTED_TARGETS) / 100, schedule: 'Four themes with nine targets each; all modes cost 1x', count: modes.length },
  ...
  revealModel: 'One deterministic reveal per round. Payout = target times mode boost. Mode-specific reveal odds preserve 96.5% RTP at a 1x cost. Maximum payout is 195x.',
  pending: ['Boosted modes above 40x payout have tail liability; some high targets have win intervals exceeding 50 plays. Dashboard limits must be evaluated for this new math model.', 'Real RGS session/currency and dashboard upload tests have not been performed.', 'Platform approval, social-mode terminology, and full jurisdiction requirements need review.'],
