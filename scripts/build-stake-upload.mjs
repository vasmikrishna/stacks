...
for (const [targetIndex, target] of SUPPORTED_TARGETS.entries()) {
  const name = targetMode(target), rows = modeBooks(target);
  ...
  modes.push({ name, cost: 1, events, weights });
  totalOutcomes += rows.length;
  const rtp = Number(wins) / Number(weight) * target;
  minRtp = Math.min(minRtp, rtp); maxRtp = Math.max(maxRtp, rtp);
  statistics.push(`${name},${(target / 100).toFixed(2)},${rtp.toFixed(9)},${Number(wins) / Number(weight)},${weight},${rows.length},0,${rows.find(row => !row.book.payoutMultiplier).book.id}`);
  if ((targetIndex + 1) % 25 === 0) console.log(`Math: ${targetIndex + 1}/${SUPPORTED_TARGETS.length} targets verified and written`);
}
...
const report = {
  ...
  targetRange: { minimum: Math.min(...SUPPORTED_TARGETS) / 100, maximum: Math.max(...SUPPORTED_TARGETS) / 100, schedule: 'Engine-compliant prediction modes with a smooth local control', count: modes.length },
  mathFiles: 1 + modes.length * 2, totalOutcomes, mathBytes,
  totalWeightPerMode: String(SAMPLE_COUNT), rtpPercentRange: [minRtp, maxRtp],
  verification: { everyModeWeightsSumTo2Pow32: true, everyModeMatchesExistingWinOdds: true, allBookCsvPayoutsMatch: true, allZstdFilesRoundTrip: true },
  revealModel: 'Representative entropy-interval midpoints, not the original full numeric reveal distribution. Target win probabilities and bonus-stage crossing probabilities are exact.',
