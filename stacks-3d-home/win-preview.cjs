...
const hook=`
 ...
 previewTools.innerHTML='<button>Preview regular</button><button>Preview big</button><button>Preview mega</button><input aria-label="Animation time" type="range" min="0" max="4000" step="50" value="1100">';
 ...
 function previewWin(target){
  ...
  winAnimation?.pause();if(winAnimation)winAnimation.currentTime=480;
 }
 previewTools.querySelectorAll('button').forEach((button,i)=>button.onclick=()=>previewWin([2.96,10,25][i]));
