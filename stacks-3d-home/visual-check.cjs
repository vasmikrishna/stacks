...
(async () => {
 ...
 try {
  ...
  await page.route('**/main.js', async route => {
   ...
   await route.fulfill({ response, body: await response.text() + `
    window.visualPreview=(kind)=>{
      clearCelebration();
      state.bet=10000;state.target=25;state.phase='running';state.started=performance.now();state.speed=0;state.breakAt=1000;
  ...
  assert(pixels>2000,'Visible illuminated crystal geometry');
  ...
  assert(await page.locator('.score-burst span').count()>0,'Score pop effect');
  ...
  console.log('PASS: illuminated canvas pixels, full tower framing, score burst, desktop/mobile screenshots, reduced motion and no runtime errors.');
